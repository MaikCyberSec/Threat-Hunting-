...
**April 2026**
- `Checkmarx KICS Supply Chain Compromise - TeamPCP Trojanized DevSecOps Scanner` — On April 22–23, 2026 Socket Research and Docker's internal monitoring disclosed that threat group **TeamPCP** had poisoned multiple distribution channels for Checkmarx KICS (Keeping Infrastructure as Code Secure), an open-source DevSecOps scanner used across CI/CD for Terraform/CloudFormation/Kubernetes. Poisoned channels: **Docker Hub** `checkmarx/kics` tags `v2.1.20`, `v2.1.20-debian`, `debian`, `alpine`, `latest`, and a fraudulent `v2.1.21`; **VS Code Marketplace + Open VSX** extensions `cx-dev-assist` v1.17.0/1.19.0 and `ast-results` v2.63.0/2.66.0; and a **backdated commit `68ed490b`** in the official Checkmarx GitHub repo hosting the stage-2 payload. Chain: trojanized KICS Go ELF (SHA256 `2a6a35f0…f146f50`) produces legitimate-looking scan output while fetching a ~10 MB obfuscated JavaScript bundle `mcpAddon.js` (SHA256 `24680027…ee4741d9`) executed via the **Bun** runtime; mcpAddon harvests GitHub tokens, AWS/Azure/GCP credentials, `.npmrc`, SSH keys, and environment variables, encrypts them, and beacons to `https://audit.checkmarx[.]cx/v1/telemetry` at `94.154.172.43` (note the `.cx` TLD — not `.com`). Stolen GitHub tokens are then used to inject malicious Actions workflow `.github/workflows/format-check.yml` abusing `${{ toJSON(secrets) }}` to serialize all repo secrets into a downloadable artifact, and stolen npm tokens to republish downstream packages. Actor stages in GitHub repos named `<word>-<word>-<3 digits>` with description "Checkmarx Configuration Storage". TeamPCP had previously hit Checkmarx, Trivy, and LiteLLM in March 2026. 13 queries covering the C2 URL/IP, the two known-bad SHA256 hashes across process/file/image-load telemetry, compromised Docker image tag pulls (docker/podman/kubectl), compromised VS Code extension folder drops, unexpected Bun runtime spawned by KICS/VS Code/node, mcpAddon.js file drops and process references, the backdoored commit `68ed490b` in command-lines and network URLs, credential-file harvest (.npmrc/.env/AWS/Azure/GCP/SSH) by kics/bun/node, `format-check.yml` workflow writes, the `toJSON(secrets)` exfil anti-pattern anywhere in the estate, CloudAppEvents GitHub audit for "Checkmarx Configuration Storage" staging repos, KICS-to-outbound-egress within a 5-minute window (catches re-hashed variants), and full risk-ranked kill-chain correlation across compromised KICS + Bun runtime + credential access + C2 egress. MITRE ATT&CK: T1195.002, T1199, T1587.001, T1583.001, T1552.001, T1552.007, T1078.004, T1567, T1567.002, T1005, T1021.004, T1087, T1557.001.
- `Anthropic Mythos Preview - Third-Party Contractor API Key Exposure` — On April 21, 2026 CybersecurityNews / Bloomberg reported that an unauthorized group demonstrated access to "Mythos," an internal Anthropic preview model the vendor had described as too dangerous to release. The group did NOT breach Anthropic's core infrastructure — they abused **shared API keys / credentials belonging to an authorized third-party contractor**, then located the preview endpoint by inferring its URL from Anthropic's known naming conventions, coordinating over a private Discord. Capabilities the group claimed to demonstrate (per Bloomberg) included autonomous zero-day discovery, multi-step exploit chaining, and sandbox escape. For defenders the relevance is NOT that your estate is "hit" — it is that this is a textbook **upstream third-party credential-exposure pattern** applied to LLM vendors, and the same class of exposure (contractor-held API keys, URL-inference to non-GA endpoints, LLM egress as a DLP blind spot) applies to every enterprise using Anthropic / OpenAI / Azure OpenAI keys. 12 queries covering full Anthropic outbound traffic baseline, Anthropic API calls from unexpected processes, `ANTHROPIC_API_KEY` / `sk-ant-` material on command lines, Anthropic credential leakage to `.env` / config files, internal / preview / non-public Anthropic model hostname references (mythos/preview/internal/staging/canary), preview-model strings in process arguments, bulk / high-volume egress to Anthropic endpoints (LLM-as-exfil-channel detection), CloudAppEvents Anthropic SaaS activity, AADSignInEventsBeta impossible-travel on federated Anthropic SSO, secret-scanner tool execution (trufflehog/gitleaks/detect-secrets/ggshield/noseyparker), Discord + Anthropic correlation (actor coordination context), and full correlation of hosts with Anthropic traffic plus any of (key-on-cmdline, secret-scanner exec, `.env` write) as a risk-ranked rotation-priority list. MITRE ATT&CK: T1078.004, T1528, T1552.001, T1552.005, T1213, T1567.002, T1589.002, T1087.004.
- `SGLang Jinja SSTI RCE - Malicious GGUF Chat Template (CVE-2026-5760)` — Critical (CVSS 9.8, CWE-94) unauthenticated RCE in the SGLang LLM/multimodal serving framework. `getjinjaenv()` uses `jinja2.Environment()` (unsandboxed) to render `tokenizer.chat_template` from the loaded GGUF model file. An attacker publishes a malicious GGUF whose chat_template contains a Jinja SSTI payload; a single `POST /v1/rerank` executes code as the SGLang process user (root in the official Docker image). Default port: 30000/tcp, default bind 0.0.0.0. Affected: SGLang ≤ 0.5.9. No vendor patch at disclosure (April 20, 2026, disclosed by CERT/CC VU#915947). Public PoC: github.com/Stuub/SGLang-0.5.9-RCE. Related unpatched SGLang RCEs: CVE-2026-3059 (ZMQ pickle), CVE-2026-3060 (encoder pickle), CVE-2026-3989 (replay_request_dump pickle). 12 queries covering SGLang inventory (TVM + process detection), SGLang launch with --model-path from untrusted sources, GGUF model downloads with origin URL tracking, direct /v1/rerank endpoint traffic, Jinja SSTI payload fingerprints (`__globals__`, `__builtins__`, `cycler.__init__`), SGLang/Python workers spawning Unix shells (post-exploitation), Cloud IMDS (169.254.169.254) access from inference hosts, Kubernetes service-account token reads, anomalous outbound egress from inference hosts (excluding HF/PyPI/PyTorch mirrors), pickle loading patterns (related CVE cluster), public PoC artifact detection (Stuub/SGLang-0.5.9-RCE), and full model-load-to-rerank-to-post-exploit kill-chain correlation. Queries are Linux-oriented (MDE for Linux / Defender for Cloud) since SGLang is overwhelmingly container-deployed.
- `FUD Crypt - Signed Malware Loader with Azure Trusted Signing Abuse` — Malware-as-a-Service crypter/loader platform (fudcrypt.net, $800–$2,000/mo) producing polymorphic, triple-layer-encrypted Windows implants. Key novelty: abuses Microsoft Azure Trusted Signing to chain payloads to the "Microsoft Identity Verification Root CA" — binaries appear Microsoft-signed and bypass SmartScreen. ~200 operators, 334 builds, 2,093 C2 commands, 32 confirmed victims in a 38-day window. Execution chain: DLL-sideloading carrier (Zoom/Slack/VS Code/OneDrive/CCleaner/ProtonVPN, plus "WindowsDF.exe" → `mpclient.dll`) → in-memory decrypt → AMSI bypass (direct AmsiScanBuffer patch OR hardware-breakpoint bypass via CPU debug registers) → ETW kill (EtwEventWrite/NtTraceEvent patch) → PEB masquerading as explorer.exe → stage-2 from Dropbox (fallback Catbox.moe) → WebSocket (WSS) C2 beacon. Infrastructure: `mstelemetrycloud.com` (C2), `fudcrypt.net` (panel). Persistence: HKCU Run key `WindowsUpdateSvc` + scheduled task `MicrosoftEdgeUpdateCore`. Agent filename: `mstelemetry.exe`. 12 queries covering the known C2/panel domains, FUD agent and carrier binary drops, mpclient.dll sideloading from non-Defender paths, legitimate carriers executing from user-writable paths, WindowsUpdateSvc registry persistence, MicrosoftEdgeUpdateCore scheduled task abuse, Dropbox/Catbox staging with rapid execution correlation, "Microsoft Identity Verification Root CA" signer on binaries in user-writable paths, known hash lookup, WebSocket C2 beacon pattern detection (interval analysis), AMSI/ETW tampering indicators, and full delivery-to-loader-to-C2 kill-chain correlation.
- `Vercel Supply Chain Breach - Context.ai OAuth Compromise & Env Var Theft` — Vercel disclosed unauthorized internal access on April 19–20, 2026 following a supply-chain compromise of Context.ai (third-party AI SaaS used by a Vercel employee). Attack chain: Context.ai compromise → malicious Google Workspace OAuth app grant → Google account takeover → lateral movement into Vercel → enumeration of environment variables not marked "sensitive" → exfiltration. ShinyHunters listed stolen data on BreachForums for $2M ransom (claimed: 580 employee records, API/npm/GitHub tokens, internal DB, source code). Vercel engaged Mandiant. Primary IOC: malicious Google OAuth client ID `110671459871-30f1spbu0hptbs60cb4vsmv79i7bbvqj.apps.googleusercontent.com`. Downstream customer risk is significant — stolen non-sensitive env vars commonly contain DB connection strings, GitHub PATs, npm tokens, AWS/GCP/Azure keys, NextAuth/JWT signing keys. 12 queries covering the malicious OAuth client ID in CloudAppEvents, Context.ai OAuth consent events, Vercel API access baselining, Context.ai network connections from endpoints, Vercel CLI env-pull usage, Vercel auth.json token file access by non-dev processes, .env file access by unexpected callers, AADSignInEvents impossible travel, new OAuth app consent grants with high-risk scopes (Gmail/Drive/Directory), credential-scanner tooling execution (trufflehog/gitleaks), high-volume Vercel API call bursts indicating token abuse, and full correlation of users exposed to BOTH Context.ai and Vercel (priority for credential rotation).
- `RedSun - Defender Cloud Files Rollback TieringEngineService LPE (Unpatched)` — Second Microsoft Defender 0-day from researcher Chaotic Eclipse (Nightmare-Eclipse), dropped publicly on April 15, 2026 — twelve days after BlueHammer. UNPATCHED as of April 17. No CVE assigned. Where BlueHammer abuses Defender's threat remediation engine, RedSun abuses Defender's cloud-file rollback mechanism. Chain: EICAR dropped via Cloud Files API (CfCreatePlaceholders) → Defender quarantines + rolls back → attacker wins oplock race → NTFS junction redirects Defender's rollback write to `C:\Windows\System32\TieringEngineService.exe` → Storage Tiers Management service executes attacker code as SYSTEM. PoC: github.com/Nightmare-Eclipse/RedSun. BlueHammer's April 15 patch does NOT mitigate RedSun. 12 queries covering TieringEngineService.exe modification (primary IOC), ImagePath registry tampering, unexpected child processes from TieringEngineService, EICAR drops in cloud-synced directories, cldapi.dll loads by untrusted callers, NTFS junction/reparse point creation targeting System32, Defender rollback + junction correlation within 5 minutes, RedSun PoC artifacts, Storage Tiers service state changes from non-trusted parents, unsigned/non-Microsoft TieringEngineService.exe on disk, low-integrity parent spawning SYSTEM child, and full Cloud-Files-to-trigger-to-binary-overwrite kill-chain correlation.
- `Interlock Ransomware - Cisco FMC Zero-Day Exploitation (CVE-2026-20131)` — Interlock ransomware group exploited CVE-2026-20131 (CVSS 10.0, insecure Java deserialization) in Cisco Secure Firewall Management Center as a zero-day since January 26, 2026 — 36 days before disclosure. Unauthenticated RCE as root on FMC appliances. Also leverages CVE-2026-20079 (CVSS 10.0, authentication bypass). Full enterprise kill chain: initial access via FMC exploit + ClickFix drive-by downloads, PowerShell RAT drops, "Chrome Updater" registry persistence, Lumma/Berserk Stealer + keylogger (klg.dll) + credential stealer (cht.exe) + Kerberoasting, lateral movement via RDP/AnyDesk/PuTTY/ScreenConnect, Cobalt Strike + SystemBC + Interlock RAT C2 (150.171.27.10), WDAC abuse to disable EDR, AzCopy exfiltration to Azure Blob Storage, AES+RSA encryption with .interlock/.1nt3rlock extensions, ransom note (!__README__!.txt) deployed via Group Policy. Known victims: DaVita, Kettering Health, Texas Tech University. CISA KEV since March 19, 2026. 13 queries covering known IOC hashes, C2 IP beacon, payload filename masquerading (conhost.exe outside System32, klg.dll, cht.exe, tmp41.wasd), ransom note and encryption artifacts, mass file encryption burst detection, "Chrome Updater" registry persistence + Startup folder drops, credential access tools and Kerberoasting, unauthorized RMM tools (AnyDesk/ScreenConnect/PuTTY) from non-standard paths, AzCopy Azure exfiltration, WDAC/EDR tampering, rundll32 DLL proxy execution, GPO-deployed ransom note propagation, and full persistence-to-credential-to-impact kill-chain correlation.
