...
**April 2026**
- `BlueNoroff Fileless PowerShell ClickFix - Crypto Targeting Campaign` — DPRK-aligned BlueNoroff (APT38 / Sapphire Sleet, Lazarus subgroup) is hunting Web3 / crypto execs through a Calendly → fake Zoom → ClickFix paste chain (Arctic Wolf, Apr 29 2026). The "SDK deprecated" overlay silently swaps the clipboard with a hidden PowerShell one-liner; pasting it drops `chromechip.log` / `chrome-debug-data001.log` in `%TEMP%` and stands up an in-memory C2 beacon checking in every ~5 s with JSON (hostname, OS, processes, admin, timezone). Persistence is a `Chrome Update Certificated.lnk` shortcut in the Startup folder. Post-compromise: browser credentials, Telegram `tdata` sessions, and live webcam frames — the latter reused to build AI-generated deepfake lures for the next victim (950+ artefacts on attacker infra; 41% of victims US, 45% C-suite, 66-day dwell). **12 KQL queries** for MDE Advanced Hunting covering the browser/Run → PowerShell paste chain, hidden-window IEX/DownloadString one-liners, the exact `chromechip.log` drop, the `Chrome Update Certificated.lnk` startup persistence (plus a broader Startup-.lnk-to-PowerShell hunt), non-MS PowerShell egress, 5-second beacon cadence binning, JSON host-fingerprint script-block sweep, browser credential-store access by PowerShell, Telegram `tdata` theft, PowerShell-driven webcam/screen capture, and full ClickFix → drop → persistence → beacon kill-chain correlation.
- `Linux Kernel "Copy Fail" - authencesn / AF_ALG LPE (CVE-2026-31431)` — Logic bug (NOT a race) in `crypto/algif_aead.c` introduced by 2017 in-place optimisation commit `72548b093ee3`. Exercising the **`authencesn`** AEAD template through the **AF_ALG** socket interface combined with **`splice()`** triggers a 4-byte out-of-bounds write at offset `assoclen + cryptlen` directly against the page cache. Disclosed by Theori's Taeyang Lee + Xint Code (Apr 29 2026); upstream patch `a664bf3d603d` (Apr 1). A 732-byte stdlib-only Python script gives **deterministic root** across Ubuntu 24.04 (6.17.0-1007-aws), Amazon Linux 2023 (6.18.8-9.213), RHEL 14.3 (6.12.0-124.45.1), SUSE 16 (6.12.0-160000.9). Page-cache-level corruption crosses container boundaries → **Kubernetes container escape** on shared nodes, and corrupted pages are NOT marked dirty so checksum-based FIM tools miss it. **12 KQL queries** for MDE for Linux covering vulnerable kernel inventory (TVM + DeviceInfo), `algif_aead` modprobe/insmod loads (excluding strongSwan / IPsec), `authencesn` / commit-hash / `CVE-2026-31431` string sweep, `/etc/modprobe.d/` tampering, setuid binary writes outside package managers, unprivileged Python → root shell within 60s, container-escape file access patterns, kernel module file events under `/lib/modules/`, public-PoC artefact references (Theori / Xint / Taeyang Lee), inverse mitigation-verification (devices missing the deny-rule), and full risk-ranked correlation. Caveat upfront: MDE for Linux can't see syscalls — augment with auditd `AF_ALG`/`splice` watches, eBPF/Falco, or SELinux audit.
